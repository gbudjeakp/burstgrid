...
variable "otelcol_version" {
  description = "OpenTelemetry Collector Contrib version embedded in the worker AMI"
  type        = string
  default     = "0.116.0"
}
...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      # OpenTelemetry Collector Contrib — config is supplied by the operator at
      # deployment time; baking the binary keeps first-worker telemetry intact.
      "curl -fsSL https://github.com/open-telemetry/opentelemetry-collector-releases/releases/download/v$${OTELCOL_VERSION}/otelcol-contrib_$${OTELCOL_VERSION}_linux_arm64.tar.gz | sudo tar -xz -C /usr/local/bin otelcol-contrib",
  ...
  # Fail the build before an incomplete or identity-bearing AMI can be shared.
  # Image-specific identities and host keys must be generated by cloud-init/sshd
  # on the launched worker, never inherited by every fleet member.
  provisioner "shell" {
    inline = [
      "set -euo pipefail",
      "test -c /dev/kvm",
      ...
      "test -s /var/lib/burstgrid/vmlinux",
      "test -s /var/lib/burstgrid/rootfs.img",
